...
export default async function AdminPage() {
  ...
  // 4. Obtener redes sociales
  const { data: socialLinks } = await supabase.from('social_links').select('*').eq('profile_id', user.id)

  return (
    <main className="min-h-screen p-4 md:p-8 max-w-6xl mx-auto">
      <AdminDashboard profile={profile} services={services || []} payments={payments} socialLinks={socialLinks || []} />
    </main>
  )
}
